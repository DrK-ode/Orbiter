...
pub fn calc_angular_velocity(
    ...
    for (mut angular_velocity, angular_acceleration) in bodies {
        **angular_velocity += **angular_acceleration * time.delta_secs();
    }
}
...
pub fn calc_direction(time: Res<Time>, bodies: Query<(&mut Direction, &AngularVelocity)>) {
    for (mut direction, angular_velocity) in bodies {
        let new_direction = Rot2::radians(**angular_velocity * time.delta_secs()) * direction.value();
        direction.assign(new_direction.fast_renormalize());
    }
}
...
pub fn limit_float<U: DerefMut<Target = f32> + Component<Mutability = Mutable>>(
    query: Query<(&mut U, &ValueLimit<U>)>,
) {
    for (mut value, value_max) in query {
        let limited = value.clamp(-value_max.limit, value_max.limit);
        **value = limited;
    }
}

pub fn limit_vec2<U: DerefMut<Target = Vec2> + Component<Mutability = Mutable>>(
    query: Query<(&mut U, &ValueLimit<U>)>,
) {
    for (mut value, value_max) in query {
        let limited = value.clamp_length_max(value_max.limit);
        **value = limited;
    }
}
