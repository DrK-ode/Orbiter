...
const noise_scale: f32 = 0.0015;
const zoom_factor: f32 = 0.4;
const miasma_speed: f32 = 0.01;
...
fn fragment(
    ...
    let zoom = (1. + zoom_factor * (mesh.world_position.z - material.min_z) / (material.max_z - material.min_z));
    let world_position = vec2(mesh.world_position.x, -mesh.world_position.y);
    let clip_position = mesh.position.xy * (1. - 1. / (2. * mesh.uv));
    let scale = noise_scale * zoom;
    var noise_value = 0.0;
    var amplitude_sum = 0.0;
    var amplitude = 1.0;
    let attenuation = 0.5;
    var fractal_factor = 1.;
    for (var i: i32 = 0; i < 5; i++){
        noise_value += amplitude * perlin_noise_2d(fractal_factor * (scale * clip_position + miasma_speed * world_position));
        fractal_factor *= 2.;
    ...
    let color_a = vec3(-0.6);
    let color_b = vec3(0.6, 0.0, 0.65);
